...
document.addEventListener('mouseup', function handleClickOutsideBox(event) {
    const box = document.getElementById('select-role');
    if (!box.contains(event.target)) {
    box.style.display = 'none';
    }
});
...
function selectAccessToStore(){
    document.getElementById('access-to-store').style.display="block"
}
//The function of closing the select category popup in   on click of anywhere on screen
document.addEventListener('mouseup', function handleClickOutsideBox(event) {
    const box = document.getElementById('access-to-store');
    if (!box.contains(event.target)) {
      box.style.display = 'none';
    }
});
